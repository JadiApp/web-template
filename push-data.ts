...
async function pushUiComponentFiles(base_url: string, unique_id: string) {
  ...
  await Promise.all(files.map(async (filepath: string) => {
    try {
      ...
      (await axios.post('/public/raw/ui-components', { filepath, data: await fsPromise.readFile(filepath, 'utf-8') }, { baseURL, params: { unique_id } })).data;
...
async function pushUiPageFiles(base_url: string, unique_id: string) {
  const baseURL = base_url;
  const files = await fg("src/pages/*.tsx");
  await Promise.all(files.map(async (filepath: string) => {
    try {
      console.log(`Push page "${filepath}...`);
      (await axios.post('/public/raw/ui-pages', { filepath, data: await fsPromise.readFile(filepath, 'utf-8') }, { baseURL, params: { unique_id } })).data;
...
async function push_data(baseURL: string, unique_id: string) {
  await pushUiComponentFiles(baseURL, unique_id);
  await pushUiPageFiles(baseURL, unique_id);
}
